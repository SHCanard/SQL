...
SELECT $__timeEpoch(EXEC_START_TIME), EXEC_NAME AS Value, CONVERT (DECIMAL(9,0),(COALESCE(ACTIONS_SUCCESS,0))) / NULLIF(CONVERT (DECIMAL(9,0),((COALESCE(ACTIONS_SUCCESS ,0) + coalesce(ACTIONS_ERROR ,0)))), 0)
FROM [dbo].[CR_STAT_EXECUTION]
